...
export class PedidoEtiquetaService extends BaseService implements IPedidoEtiquetaService {
  ...
  async provisionarEtiqueta(params: ProvisionarEtiquetaDTO): Promise<ResponsePadrao<void>> {
  ...
  async anexarEvento(params: AnexarEventoDTO): Promise<ResponsePadrao<void>> {
    await new Promise(resolve => setTimeout(resolve, 1500));
